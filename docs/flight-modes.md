...
| 形态 | 飞控模式 | 含义 |
|------|----------|------|
| 垂起 | **仅** `QSTABILIZE` | 悬停姿态自稳；**不用** `QHOVER`；尾桨仅在此模式、解锁后正转 |
| 固飞 | `STABILIZE` | 飞控自稳（杆回中回平） |
| 固飞 | `MANUAL` | 纯手动直通 |
| 过渡 | 无独立开关档 | 由**形态开关**切换触发 |

本机正式档位不含 `QHOVER`、`FBWA`。机尾俯仰小电机**仅**在 `QSTABILIZE` 解锁后正转（PWM ≥ 中位，随油门增速）；进入固飞立即停转。
...
- **固飞 → 垂起**：形态开关切垂起 → 进入 `QSTABILIZE` → 后飞/悬停过渡。扫角起点尾桨只出低头前馈（`BPIT_TFF`），不跟姿态环抬头；倾转到垂直后交叉淡入环路。见 [ardupilot-setup.md](./ardupilot-setup.md) §3 / §4.4。
